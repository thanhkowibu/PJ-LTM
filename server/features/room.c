...
int create_room(const char *name, int capacity) {
...
int add_user_to_room(const char *room_name, User *user) {
    pthread_mutex_lock(&room_mutex);
    if (user == NULL) {
        if ((user = getCurUser()) == 0) { // User = null thi cho la curUser
            pthread_mutex_unlock(&room_mutex);
            return 0; // No user provided
        };
    }
...
int delete_user_from_room(const char *room_name, User *user) {
    pthread_mutex_lock(&room_mutex);
    if (user == NULL) {
        if ((user = getCurUser()) == 0) { // User = null thi cho la curUser
            pthread_mutex_unlock(&room_mutex);
